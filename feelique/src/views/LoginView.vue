<template>
  <div class="auth-page">
    <div class="auth-container">
      <h1>Anmelden</h1>
      <p class="subtitle">Willkommen zurück bei Feelique</p>
      ...
      <p class="switch-auth">
        Noch kein Konto? <a href="#" @click.prevent="$router.push('/register')">Registrieren</a>
      </p>
    </div>
  </div>
</template>
    ...
    async login() {
      try {
        ...
        if (response.ok && user.sessionToken) {
          // Token und E-Mail speichern
          localStorage.setItem("sessionToken", user.sessionToken);
          localStorage.setItem("userEmail", user.email);

          // NEU: Namen speichern (falls vorhanden)
          if (user.firstName) {
          ...
          console.log("Login erfolgreich:", user); // Debug-Ausgabe
